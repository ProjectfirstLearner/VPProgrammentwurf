...
uint32_t gasValue1 = 0;
uint32_t gasValue2 = 0;
...
int32_t DualChannelSetVoltage(){
	...
	int32_t adcValue1 = adcReadChannel(ADC_INPUT0);
	...
	int32_t adcValue2 = adcReadChannel(ADC_INPUT1);
...
int32_t DualChannelVoltageAverage(uint32_t *avrgValue){

	if (avrgValue == NULL)return SENSOR_INVALID_PTR;
	...
	*avrgValue = (gasValue1 + gasValue2)/COUNT_SENSORS;
